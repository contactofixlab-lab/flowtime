...
export default function CalendarClient({ events, tasks }: Props) {
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", padding: "24px 16px 0" }}>
      ...
      {/* Grid */}
      <div style={{ ...card, padding: 8, marginBottom: 20 }}>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  alignItems: "center", justifyContent: "center", borderRadius: 12, border: "none",
                  cursor: "pointer", fontSize: 13, fontWeight: sel || tod ? 700 : 500, fontFamily: "inherit",
                  background: sel ? "linear-gradient(135deg,#4F46E5,#7C3AED)" : tod ? "rgba(79,70,229,0.15)" : "transparent",
                  color: sel ? "#fff" : tod ? "#818CF8" : "#94A3B8",
                  boxShadow: sel ? "0 4px 16px rgba(79,70,229,0.4)" : "none",
                }}
              >
                {format(day, "d")}
                {(hasE || hasT) && (
                  <div style={{ position: "absolute", bottom: 3, display: "flex", gap: 2 }}>
                    {hasT && <span style={{ width: 4, height: 4, borderRadius: "50%", background: "#818CF8" }} />}
                    {hasE && <span style={{ width: 4, height: 4, borderRadius: "50%", background: "#34D399" }} />}
                  </div>
                )}
              </button>
            );
      ...
      {/* Detalle del día */}
      <h2 style={{ color: "#64748B", fontSize: 12, fontWeight: 700, margin: "0 0 10px", letterSpacing: "0.05em", textTransform: "uppercase" }}>
      ...
      {selectedTasks.length === 0 && selectedEvents.length === 0 ? (
        <div style={{ ...card, padding: "28px 20px", textAlign: "center" }}>
          <p style={{ color: "#334155", fontSize: 13, margin: 0 }}>Sin tareas ni eventos este día</p>
        </div>
      ) : (
